...
Page({
  ...
  // 执行转换
  convertText() {
    const { currentConverter, input, isEncoding } = this.data;
    
    if (!input.trim()) {
      this.setData({ output: '', errorMessage: '' });
      return;
    }
  ...
  // 分享功能
  onShareAppMessage() {
    const { currentConverter, isEncoding, output } = this.data;
    const converterName = this.data.converterTypes[this.data.converterIndex];
    const action = isEncoding ? '编码' : '解码';
    
    return {
      title: `${converterName}${action}结果`,
      path: '/pages/index/index',
      desc: '编码转换工具处理结果',
      content: output.length > 100 ? output.substring(0, 100) + '...' : output
    };
  }
});
